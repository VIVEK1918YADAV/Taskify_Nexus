...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // Force data refresh when component mounts to avoid stale data
    refetch();
    ...
    <div className='h-full py-4'>
      <>
        ...
        <div className='w-full flex flex-col md:flex-row gap-4 2xl:gap-10 py-8'>
          ...
          {/* RECENT USERS */}
          {data && (user?.isAdmin || user?.isManager) && <UserTable users={data?.users} />}
        </div>
      </>
    </div>
  );
    ...
    <tr className='border-b border-gray-200 text-gray-600 hover:bg-gray-300/10'>
      ...
      <td className='py-2 hidden md:block'>
        <span className='text-base text-gray-600'>
          {task?.createdAt ? moment(new Date(task.createdAt)).fromNow() : moment(new Date(task?.date)).fromNow()}
        </span>
      </td>
    </tr>
  );
    ...
    <>
      <div
        className={clsx(
          ...
          user?.isAdmin || user?.isManager ? "md:w-2/3" : ""
        )}
      >
